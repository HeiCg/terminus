...
| --- | --- | --- | --- | --- |
| `PORT` | `8787` | collector | Loopback UI / `/api/*` / exports HTTP port. Read as a bare name (no `TERMINUS_` prefix). | — |
| `INGEST_PORT` | `8788` | collector | WSS device-capture ingest port (`/ingest`, TLS on the LAN). Bare name. | — |
| `ATLANTIS_PORT` | `10909` | collector | Atlantis TLS capture ingest port. Bare name. | — |
...
| `TERMINUS_LOG_LEVEL` | `info` | collector | Log verbosity (`debug`\|`info`\|`warn`\|`error`). **Since next release** — the ops round adds it; the collector on the current release does not yet read it. | `NETCAPTURE_LOG_LEVEL` |
...
- **Deprecated aliases.** Every variable the collector reads through its `env()`
  ...
  one-time warning per name. The bare port variables (`PORT`, `INGEST_PORT`,
  `ATLANTIS_PORT`) are read directly and accept only those exact names. The CLI
